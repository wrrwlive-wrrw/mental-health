...
【危险信号】如有红旗征象请重点标注`;
  ...
  try {
    const resp = await fetch(AI_CONFIG.apiUrl, {
      ...
      body:JSON.stringify({model:AI_CONFIG.model, messages, max_tokens:3500, temperature:0.3})
    });
    const data = await resp.json();
    let reply = data.choices?.[0]?.message?.content||'分析失败';
    // 清理思考标签（兼容DeepSeek/Qwen等CoT模型）
    ...
    if (!reply || reply.length < 10) reply = '诊断分析生成失败，请检查API配置后重试。';
